...
pc=Pinecone(api_keys=os.environ.get("PINECONE_API_KEY"))

# initialize pinecone database
index_name=os.environ.get("PINECONE_INDEX_NAME")

# check whether index exists, and create if not
exisiting_indexes=[index_info["name"] for index_info in pc.list_indexes()]

if index_name not in exisiting_indexes:
    pc.create_index(
        name=index_name,
        dimension=3072,
        metric="cosine",
